...
use crate::layers::{business::{shared::{business_rules::{FieldValidationResult, UseCaseInputValidationResult, UseCaseInvalidInputResult}, errors::{BusinessError, InternalDependencyError, UseCaseError}, validation_rules::{rtsp_url::rtsp_url, strings::non_empty}}, usecases::put_camera::{interface::{IPutCameraUseCase, PutCameraInput, PutCameraOutput}, sanitization_rules::PutCameraSanitizedInput}}, ewm::{main_database::qc_collection::camera_qc_collection::{ICameraQCCollection, PutCameraCommandInput}, permanent_stream_server::IPermanentStreamServer}};
...
where
    ...
    IIPermanentStreamServer: IPermanentStreamServer,
{
    pub fn new(camera_qc_collection: IICamercaQCCollection, permanent_stream_server: IIPermanentStreamServer) -> Self {
        Self { camera_qc_collection, permanent_stream_server }
    }
...
impl<IICamercaQCCollection, IIPermanentStreamServer> IPutCameraUseCase for
    PutCameraUseCase<IICamercaQCCollection, IIPermanentStreamServer>
where
    ...
    IIPermanentStreamServer: IPermanentStreamServer + Sync,
{
    ...
    async fn execute(&self, input: PutCameraInput) -> Result<PutCameraOutput, UseCaseError> {
        ...
        let update_camera_command_result = self.camera_qc_collection.put_camera(update_camera_command).await.map_err(|e| {
            UseCaseError::InternalDependencyError(InternalDependencyError::new(
